...
Context
  ...
  {lowering_options : Type}
  (aparams : architecture_params_wasm)
  (cparams : compiler_params lowering_options).

Locate architecture_params.

Notation saparams := (ap_sap_wasm aparams).
(* Notation liparams := (ap_lip_wasm aparams). *)
(* Notation loparams := (ap_lop_wasm aparams). *)
Notation shparams := (ap_shp_wasm aparams).
Notation agparams := (ap_agp_wasm aparams).
...
Definition live_range_splitting (p: uprog) : cexec uprog :=
  ...
  Let pv := dead_code_prog (ap_is_move_op_wasm aparams) pv false in
...
Definition compiler_first_part (to_keep: seq funname) (p: uprog) : cexec uprog :=
  ...
  Let p := unroll_loop (ap_is_move_op_wasm aparams) p in
...
Definition compiler_third_part (returned_params: funname -> option (seq (option nat))) (ps: sprog) : cexec sprog :=
  ...
  in
  Let pr := dead_code_prog_tokeep (ap_is_move_op_wasm aparams) false rminfo ps in
  ...
  Let pd := dead_code_prog (ap_is_move_op_wasm aparams) pa true in
...
Definition compiler_back_end entries (pd: sprog) :=
  ...
  in
  Let pl := stack_zeroization_lprog aparams.(ap_szp_wasm) szs_of_fn pl in
