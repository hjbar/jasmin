...
let compile (type reg regx xreg rflag cond asm_op extra_op)
  ...
  (*
  ignore (export_functions, cparams);
  failwith "Compiler_wasm.compile_prog_to_asm TODO"
  *)

  (* TODO : finish the compilation *)
  (*
  Compiler_wasm.compile_prog_to_asm Arch.asm_e Arch.call_conv Arch.aparams cparams
    export_functions
