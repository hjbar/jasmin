...
run_benchmark() {
  ...
  res_x86=$(taskset --cpu-list 0 "$f_exe" "$NB_REPEAT" "$NB_ITER" "$STREAM_LENGTH" "$KEY" "$NONCE" "$VERBOSE")
  ...
  res_wasm_wat2wasm=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$STREAM_LENGTH" "$KEY" "$NONCE" "$VERBOSE")
  ...
  if [ "$ALL_OPT" = true ]; then
    ...
    res_wasm_wasmas=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$STREAM_LENGTH" "$KEY" "$NONCE" "$VERBOSE")
...
run_benchmark $NB_REPEAT 59000000   "32" "$NONCE" "$KEY"
run_benchmark $NB_REPEAT 15800000  "256" "$NONCE" "$KEY"
run_benchmark $NB_REPEAT  2000000 "2048" "$NONCE" "$KEY"
