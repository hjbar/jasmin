...
extern int jade_stream_chacha_chacha20_amd64_ref(uint8_t *stream, uint64_t stream_length, const uint8_t *nonce, const uint8_t *key);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < warmup_iter; i++) {
    jade_stream_chacha_chacha20_amd64_ref(stream, length, nonce, key);
  }
