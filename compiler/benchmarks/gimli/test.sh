...
run_benchmark() {
  ...
  res_x86=$(taskset --cpu-list 0 "$f_exe" "$NB_REPEAT" "$NB_ITER" "$VERBOSE")
  ...
  res_wasm_wat2wasm=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$VERBOSE")
  ...
  if [ "$ALL_OPT" = true ]; then
    ...
    res_wasm_wasmas=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$VERBOSE")
...
run_benchmark $NB_REPEAT 31500000
