...
run_benchmark() {
  ...
  res_x86=$(taskset --cpu-list 0 "$f_exe" "$NB_REPEAT" "$NB_ITER" "$INPUT" "$VERBOSE")
  ...
  res_wasm_wat2wasm=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$INPUT" "$VERBOSE")
  ...
  if [ "$ALL_OPT" = true ]; then
    ...
    res_wasm_wasmas=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$INPUT" "$VERBOSE")
...
run_benchmark $NB_REPEAT 20000000 "$MSG_01"
run_benchmark $NB_REPEAT  2420000 "$MSG_02"
run_benchmark $NB_REPEAT   344000 "$MSG_03"
