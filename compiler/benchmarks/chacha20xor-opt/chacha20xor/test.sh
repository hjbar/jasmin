...
run_benchmark() {
  ...
  local LOG_FILE="$LOG_DIR/log_${name}_${NB_REPEAT}_${NB_ITER}.txt"
  echo "$NAME" > "$LOG_FILE"

  printf "\n%s\n" "$SEP2" | tee -a "$LOG_FILE"

  res_x86=$(taskset --cpu-list 0 "$f_exe" "$NB_REPEAT" "$NB_ITER" "$MSG" "$KEY" "$NONCE")
  ...
  res_wasm_wat2wasm=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$MSG" "$KEY" "$NONCE")
  ...
  wasm-as --enable-multivalue "$f_wat" -o "$f_wasm"
  res_wasm_wasmas=$(taskset --cpu-list 0 node "$f_js" "$NB_REPEAT" "$NB_ITER" "$MSG" "$KEY" "$NONCE")
  ...
  if [ -z "$time_x86" ] || [ -z "$time_wasm_wat2wasm" ] || [ -z "$time_wasm_wasmas" ]; then
...
run_benchmark $NB_REPEAT 59330000 "$MSG_01" "$NONCE" "$KEY"
run_benchmark $NB_REPEAT 15800000 "$MSG_02" "$NONCE" "$KEY"
run_benchmark $NB_REPEAT  2000000 "$MSG_03" "$NONCE" "$KEY"
