...
if (process.argv.length < 7) {
  console.log("Usage: node %s <nb_repeat> <nb_iterations> <input> <input_nonce> <input_key>", process.argv[1]);
  process.exit(1);
}
...
async function runWasm() {
  // Init args
  const wasmPath = path.join(__dirname, 'prog/chacha20xor_wasm.wasm');
  ...
  for (let i = 0; i < WARMUP_ITER; i++) {
    exports.jade_stream_chacha_chacha20_amd64_ref_xor(pOut, pIn, len, pNonce, pKey);
  }
