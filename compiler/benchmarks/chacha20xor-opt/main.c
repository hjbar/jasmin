...
extern int jade_stream_chacha_chacha20_amd64_ref_xor(uint8_t *output, const uint8_t *input, uint64_t input_length, const uint8_t *nonce, const uint8_t *key);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < warmup_iter; i++) {
    jade_stream_chacha_chacha20_amd64_ref_xor(output_bytes, input_bytes, length, nonce, key);
  }
