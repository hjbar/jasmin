...
extern int jade_stream_chacha_chacha20_amd64_avx_xor(uint8_t *output, const uint8_t *input, uint64_t input_length, const uint8_t *nonce, const uint8_t *key);
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 7) {
    printf("Usage: %s <nb_repeat> <nb_iterations> <input> <input_nonce> <input_key> <verbose>\n", argv[0]);
    return 1;
  }
  ...
  const uint64_t length = strlen(input_raw);
  uint8_t *input_bytes  = malloc(length > 0 ? length : 1);
  uint8_t *output_bytes = malloc(length > 0 ? length : 1);
  ...
  for (int i = 0; i < warmup_iter; i++) {
    jade_stream_chacha_chacha20_amd64_avx_xor(output_bytes, input_bytes, length, nonce, key);
  }
  ...
  if (VERBOSE) {
    printf("Result        : "); for (int i = 0; i < length; i++) printf("%02x", output_bytes[i]); printf("\n");
  }
